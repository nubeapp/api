...
from ..models import AssistantBase, AssistantResponse, AssistantTicketResponse, UserRequest
...
@router.get("/{event_id}", response_model=List[AssistantTicketResponse])
async def get_assistants_by_event_id(event_id: int, db: Session = Depends(get_db), current_user: int = Depends(get_current_user)):
    assistants = db.query(schemas.Assistant).join(schemas.Ticket).filter(
        schemas.Ticket.event_id == event_id,
        schemas.Ticket.event.has(id=event_id)
    ).all()
    return assistants
